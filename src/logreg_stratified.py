...
def save_results(models, metrics, ensemble_metrics, simple_ensemble_metrics, control_only, male, female):
  """
...
def create_model_and_metrics(X_dict, y, site_data, num_splits, num_repeats, random_ints, permute):
  ...
  ensemble_metrics = {"accuracies": np.empty(num_repeats),
                      "balanced_accuracies": np.empty(num_repeats), 
                      "roc_aucs": np.empty(num_repeats),
                      "coefficients": np.empty((num_repeats*num_splits, 4)), # 4 base models
                      "all_true_labels": [],
                      "all_pred_probs": []} 
  ...
  for repeat_idx in range(num_repeats):
    ...
    ensemble_outer_loop_metrics = {"accuracies": np.empty(num_splits),
                                    "balanced_accuracies": np.empty(num_splits),
                                    "roc_aucs": np.empty(num_splits),
                                    "coefs": np.empty((num_splits, 4))} # 4 base models 
    ...
    ensemble_metrics['coefficients'][(repeat_idx*num_splits):(repeat_idx*num_splits)+5, :] = ensemble_outer_loop_metrics['coefs']
...
if __name__ == "__main__":
  ...
  # X_dict = {model: scale_data(np.load(f'data/training_data/aligned/X_{model}_{file_name}{sex}.npy')) for model in MODELS}
  X_dict = {'SC': scale_data(np.load(f'data/training_data/aligned/X_SC_{file_name}{sex}.npy')),
            'FC': scale_data(np.load(f'data/training_data/aligned/X_FC_{file_name}{sex}.npy')),
            'FCgsr': scale_data(np.load(f'data/training_data/aligned/X_FCgsr_{file_name}{sex}.npy')),
            'demos': scale_data(np.load(f'data/training_data/aligned/X_demos_{file_name}{sex}_no_sex.npy'))
            }
  ...
  if SAVE_RESULTS and not PERMUTE:
    ...
    save_results(models=MODELS, metrics=metrics, ensemble_metrics=ensemble_metrics, simple_ensemble_metrics=simple_ensemble_metrics, control_only=CONTROL_ONLY, male=MALE, female=FEMALE)
